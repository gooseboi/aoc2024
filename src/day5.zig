...
const ParseResult = struct {
    rules: ArrayList(Tuple(&[_]type{ u32, u32 })),
    input: ArrayList(ArrayList(u32)),
};

fn parse(s: []const u8, alloc: std.mem.Allocator) !ParseResult {
    var lines = std.mem.splitScalar(u8, s, '\n');
    var parsing_rules = true;
    var rules = ArrayList(Tuple(&[_]type{ u32, u32 })).init(alloc);
    var input = ArrayList(ArrayList(u32)).init(alloc);
    while (lines.next()) |line| {
        ...
        if (parsing_rules) {
            var line_vals = std.mem.splitScalar(u8, line, '|');
            const val1 = try std.fmt.parseUnsigned(u32, line_vals.next().?, 10);
            const val2 = try std.fmt.parseUnsigned(u32, line_vals.next().?, 10);

            try rules.append(.{ val1, val2 });
        } else {
            var items = std.mem.splitScalar(u8, line, ',');
            var tmp = ArrayList(u32).init(alloc);
            while (items.next()) |item| {
                const val = try std.fmt.parseUnsigned(u32, item, 10);
                try tmp.append(val);
            }
            try input.append(tmp);
        }
    }
